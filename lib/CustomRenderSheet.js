...
var CustomRenderSheet = function (_PureComponent2) {
	...
	function CustomRenderSheet(props) {
		...
		_this2.state = {
			...
			grid: [[{ name: '', value: '' }, { name: '', value: '' }, { name: '', value: '' }, { name: '', dataEditor: _DataSelect2.default }]]
	...
	_createClass(CustomRenderSheet, [{
		...
		value: function checkDataTypeSelect(i, j) {
			...
			var select = Object.keys(grid[i][j]).indexOf('select') > -1 ? true : false;
			if (select) {
				grid[i][j] = { name: '', value: '', dataEditor: _DataSelect2.default };
			}
		}
	}, {
		key: 'delete',
